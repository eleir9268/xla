  ...
  for (const auto& minimal_reshape : reshapes) {
    switch (minimal_reshape.category) {
      ...
      case MinimalReshapeCategory::kDecreaseRank: {
        int64_t source_id = GetFirstNonTrivialDimId(
            source_shape, minimal_reshape.input_dim_ids);
        int64_t target_id = GetFirstNonTrivialDimId(
            target_shape, minimal_reshape.output_dim_ids);
