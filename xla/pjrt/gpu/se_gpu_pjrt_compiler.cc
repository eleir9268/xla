...
// Returns a copy of the `gpu_topology` with the `target_config` field set to
// `gpu_target_config`.
GpuTopology CopyTopologyWithTargetConfig(
    const GpuTopology& gpu_topology,
    const gpu::GpuTargetConfig& gpu_target_config) {
  return GpuTopology(gpu_topology.platform_version(),
                     gpu_topology.num_partitions(),
                     gpu_topology.num_hosts_per_partition(),
                     gpu_topology.num_devices_per_host(), gpu_target_config,
...
absl::StatusOr<GpuTopology> GetTopologyWithTargetConfig(
  ...
  if (gpu_topology_description->target_config().has_value()) {
    ...
    return CopyTopologyWithTargetConfig(
        gpu_topology_description->gpu_topology(), gpu_target_config);
  }
  ...
  if (options.gpu_target_config.has_value()) {
    ...
    return CopyTopologyWithTargetConfig(
        gpu_topology_description->gpu_topology(),
